...
replace_na_with_value <- function(vector, value, dbg = TRUE)
{
  ...
  if (length(na_indices)) {
    kwb.utils::catAndRun(
      sprintf("Replacing %d NA values with %s", length(na_indices), value),
      expr = {
        vector[na_indices] <- value
      },
      dbg = dbg
    )
  }
...
replace_indices_with_value <- function(vector, indices, value, dbg = TRUE)
{
  ...
  if (n_indices > 0L) {
    kwb.utils::catAndRun(
      sprintf("Replacing %d values with %s", n_indices, value),
      expr = {
...
{
  available_multi_cols <- names(df) %>%
    stringr::str_extract(".*[0-9][0-9]$") %>%
    stringr::str_remove("[0-9][0-9]$") %>%
    unique()

  available_multi_cols[!is.na(available_multi_cols)]
}
...
tidy_selected_cols <- function(df, column = "author")
{
  cols_selected <- c(
    "rec_number",
    select_columns(df, pattern = sprintf("^%s[0-9][0-9]", column))
  )

  if (length(cols_selected) > 1L) {
    tidy_df(df[,cols_selected], exclude_cols = "rec_number") %>%
      tidyr::separate(
        .data$key,
    ...
    stop(sprintf(
      "Provided multi column '%s' not 'df'\nPlease choose one of: %s",
      column,
...
tidy_multi_cols_list <- function(df)
{
  ...
  list(
    author = tidy_selected_cols(df, valid_multi_cols[3]),
